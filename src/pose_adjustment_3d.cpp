...
void readG2OFile(const std::string& filename, NonlinearFactorGraph& graph, Values& initialEstimate){
    ...
    while (getline(infile, line)) {
        ...
        if (tag == "VERTEX_SE3:QUAT") {
            ...
            graph.emplace_shared<BetweenFactor<Pose3>>(Symbol('x', id1), Symbol('x', id2), pose, model);
        }
    }
...
int main(int argc, char** argv) {
    ...
    // You should change the filename to the path of the dataset on your machine
