本作業手順書は、下記のURLで示される「SLAMを実装するためのサンプルコード」を実行するためのものである。
https://github.com/TakuOkawara/pose_adjustment_demo

# 作業手順書

　本セミナーのデモで必要なソフトウェアをインストールおよび実行するための手順を下記に示す。下記の手順は、全てUbuntu 24.04の環境での実行を前提とする。Ubuntu 24.04の環境は、例えば[この記事](https://tech-landlord.com/articles/dell-notepc-dual-boot-win11-ubuntu/)などを参考に構築することができる。
...
## gtsamのインストール
　gtsamは、ファクタグラフの構築および最適化計算を実装するためのライブラリとなる。
...
## iridescenceのインストール
　iridescenceは、地図や自己位置の可視化で使われるライブラリとなる。本サンプルプログラムでは、SLAMで得られた軌跡を可視化する。

 [iridescenceのレポジトリの「C++ : Install from PPA (Ubuntu)」](https://github.com/koide3/iridescence#c--install-from-ppa-ubuntu)に従って、必要なライブラリを揃えた上でコンパイルのコマンドを実行する。具体的には、下記のコマンドを実行する。
```commandline
