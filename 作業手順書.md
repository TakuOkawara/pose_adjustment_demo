...
このとき、下記の画像の **[CMakeLists.txt](https://github.com/koide3/iridescence/blob/master/CMakeLists.txt)の4行目のオプションをOFFからONに変更し、examplesのコードを実行可能に変更する**。もしくは、「cmake ..」を実行するとき。「cmake .. -BUILD_EXAMPLES=ON」というようにオプションをつけてコマンドを実行すればよい。
